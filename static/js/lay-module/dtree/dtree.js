...
layui.define(["jquery", "layer", "form"], function (exports) {
  ...
  var AjaxHelper = {
    request: function (config) {
      ...
      console.log("yy", config.contentType)
      $.ajax({
  ...
  DTree.prototype.checkAllOrNot = function ($i) {
    ...
    if ($i.attr("data-checked") == "1") {
      ...
      for (var i = 1, item = $parent_li; i < item.length; i++) {
        var flag = item
          .eq(i)
          .find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "'][data-checked='1']")
          .length
        if (flag == 0) {
          //把父级去掉选中
          var $item_i = item
    ...
    if ($i.attr("data-checked") == "1") {
      ...
      for (var i = 1, item = $parent_li; i < item.length; i++) {
        var flag1 = item
          ...
          .find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "'][data-checked='1']")
          .length
        var flag2 = item.eq(i).find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "']")
          .length
        var $item_i = item.eq(i).find(">." + LI_DIV_ITEM + ">." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "']")
        if (flag1 != flag2) {
          // 父级复选框半选
    ...
    return {
      setToolbarOption: function (toolbarId, title, classId, other) {
        if (toolbarWay == "contextmenu") {
          return (
            "<dd><a dtree-tool='" + toolbarId + "'><i class='" + classId + "'></i>&nbsp;" + other + title + "</a></dd>"
          )
  ...
  var dtree = {
    ...
    click: function (dTree, id) {
      ...
      $(
        "div[dtree-click='" + eventName.itemNodeClick + "'][dtree-id='" + dTree.obj[0].id + "'][data-id='" + id + "']"
      ).click()
    },
